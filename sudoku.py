...
class sudoku_game():
    outArray = [[0 for x in range(9)] for x in range(9)]
    answerArray = [[0 for x in range(9)] for x in range(9)]
    def play_sudoku(self):
      args = sys.argv[1:]
      if len(args) > 0:
        puzzles = [loadboard(filename) for filename in args]
      else:
        puzzles = [self.makepuzzle(self.solution([None] * 81))]
      for puzzle in puzzles:
        print "PUZZLE:"
        print self.printboard(puzzle)
        print "RATING:", self.ratepuzzle(puzzle, 4)
        #if len(args) > 0:
        #  print
        print "SOLUTION:"
        answer = self.solution(puzzle)
        if answer is None: print "NO SOLUTION"
        else: print self.answerboard(answer)
    ...
    def cgimain():
      ...
      if digits:
        ...
        puzzle = self.makepuzzle(solution([None] * 81))
      if ext in ['pdf', 'ps']:
        puzzle = [puzzle[x % 9 + 72 - (x - x % 9)] for x in xrange(81)]
      ratingstr = '%1.2f' % self.ratepuzzle(puzzle, 4)
    ...
    #outArray = [[0 for x in range(10)] for x in range(10)]
      for row in xrange(9):
        for col in xrange(9):
        #out += (""," "," ","  "," "," ","  "," "," ")[col]
        #out += self.printcode(board[self.posfor(row, col)])
        #out += ('\n','\n','\n\n','\n','\n','\n\n','\n','\n','\n')[row]
            self.outArray[row][col] = self.printcode(board[self.posfor(row, col)])
      return self.outArray

    def answerboard(self, board):
        for row in xrange(9):
            for col in xrange(9):
                self.answerArray[row][col] = self.printcode(board[self.posfor(row, col)])
